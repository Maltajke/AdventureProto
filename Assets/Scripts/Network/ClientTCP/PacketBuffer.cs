...
namespace Capybara.Network
{
    public class PacketBuffer : IDisposable
    {
        ...
        {
            var value = new Vector3 {x = ReadFloat(), y = ReadFloat(), z = ReadFloat()};
            return value;
        }

        public Vector2 ReadVector2(bool peek = true)
        {
            var value = new Vector2 {x = ReadFloat(), y = ReadFloat()};
            return value;
        }

        public Quaternion ReadQuaternion(bool peek = true)
        {
            var value = new Quaternion {x = ReadFloat(), y = ReadFloat(), z = ReadFloat(), w = ReadFloat()};
            return value;
        }
